...
namespace Rayforge.Core.Environment.Spatial.Chunks
{
    ...
    /// </para>
    /// </summary>
    public struct GridRadiusEdgeState : IIterationLogic<Vector3Int, GridRadiusEdgeState>
    {
        ...
        /// </summary>
        public bool MoveNext(ref GridRadiusEdgeState self, out Vector3Int result)
        {
        ...
        /// </summary>
        public bool TryPeekNext(ref GridRadiusEdgeState self, out Vector3Int result)
        {
            CalculateNext(ref self);
            result = self._cachedValue;
        ...
        /// </summary>
        public bool HasNext(ref GridRadiusEdgeState self)
        {
            CalculateNext(ref self);
            return self._hasCachedValue;
        }
        ...
        /// </remarks>
        private static void CalculateNext(ref GridRadiusEdgeState self)
        {
            ...
            while (self._rangeState.MoveNext(ref self._rangeState, out Vector3Int candidate))
            {
                ...
                if (SpatialUtils.GetSqrDistanceEdge(self._localCentre, cellPos, self._halfSizes, self._activeAxes) <= self._sqrRadius)
                {
